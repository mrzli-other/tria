...
final class PowerUpAntiGravityCharacterState extends CharacterStateBase {
    
    private static final float SPEED = GameCharacter.JUMP_SPEED * 0.7f;
    private static final float RELEASE_SPEED = GameCharacter.JUMP_SPEED * 1.0f;
    private static final float MIN_SPEED = 5.0f;
    private static final float ACCELERATION = 25.0f;
    ...
    public PowerUpAntiGravityCharacterState(CharacterStateManager characterStateManager, CareerData careerData, AssetManager assetManager) {
        super(characterStateManager);
        
        characterGraphics = new CharacterGraphicsComposition(gameCharacter, careerData, true, true, false, assetManager);
    ...
    public void start(CharacterStateChangeData changeData) {
        Sounds.loop(ResourceNames.SOUND_ANTI_GRAVITY);
    }
    
    @Override
    public void end() {
        Sounds.stop(ResourceNames.SOUND_ANTI_GRAVITY);
    }
    ...
    public void update(CharacterStateUpdateData updateData) {
        ...
        if (!characterEffects.isAntiGravity()) {
            ...
            if (isPlatformInRange) {
                ...
                characterEffects.setAntiGravityAdditionalDuration();
            }
        }
    ...
    private float getSpeed(float currentSpeed, float delta) {
        float remainingDuration = gameCharacter.getCharacterEffects().getAntiGravityRemaining();
